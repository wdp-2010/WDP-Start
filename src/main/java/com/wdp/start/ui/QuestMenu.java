...
public class QuestMenu {
    ...
    public void handleClick(Player player, int slot, Inventory inv) {
        ...
        if (!data.isStarted() && (slot == 31 || slot == 22)) {
            player.closeInventory();
            // Force-start to skip "already experienced" guard so new players can always begin
            plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
                plugin.getQuestManager().startQuests(player, true);
            }, 1L);
            return;
        }
